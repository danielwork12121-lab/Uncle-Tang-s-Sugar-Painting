...
// ================================================================
const SCENE7_GRID_RESOLUTION_MULTIPLIER = 2;  // Double the grid resolution
const SCENE7_SPREAD_STRENGTH = 0.55;          // Lower = more precise (0.55 = 55% of original)

// Drawing grid constants (doubled resolution for more precise drawing)
const GRID_COLS = 27 * SCENE7_GRID_RESOLUTION_MULTIPLIER;  // = 54
const GRID_ROWS = 30 * SCENE7_GRID_RESOLUTION_MULTIPLIER;  // = 60
...
const SCENE7_MAX_VISUAL_THICKNESS = 3.5; // Cap at warm golden/orange, not brown
...
const SCENE7_SYRUP_VOLUME = 0.225;

// Task A: Button 5 BGM volume
const SCENE7_BGM_VOLUME = 0.18;

// Task B: Production quantity config
const SCENE7_MIN_PRODUCTION_QUANTITY = 1;
const SCENE7_MAX_PRODUCTION_QUANTITY = 1; // Max 1 dragon candy
const SCENE7_REQUIRED_QUANTITY = 1; // Customer wants exactly 1

// ================================================================
//  Dragon bitmap mask — directly from 54×60 bitmap
// ================================================================

// Dragon bitmap (54 columns × 60 rows)
// '#' = target blueprint cell, '.' = empty cell
const SCENE7_DRAGON_BITMAP = [
  "......................................................",
  "......................................................",
  "......................................................",
  "................######................................",
...
// Validate dragon bitmap dimensions
if (SCENE7_DRAGON_BITMAP.length !== GRID_ROWS) {
  console.warn("Scene7 dragon bitmap row count mismatch", SCENE7_DRAGON_BITMAP.length, GRID_ROWS);
}

for (let r = 0; r < SCENE7_DRAGON_BITMAP.length; r++) {
  const row = SCENE7_DRAGON_BITMAP[r];
  if (row.length !== GRID_COLS) {
    console.warn("Scene7 dragon bitmap column count mismatch at row", r, ":", row.length, GRID_COLS);
  }
  for (let c = 0; c < row.length; c++) {
    if (row[c] !== '#' && row[c] !== '.') {
      console.warn("Scene7 dragon bitmap invalid character at row", r, "col", c, ":", row[c]);
    }
  }
}

// Build dragon mask from bitmap
// Returns a 2D array (rows × cols) with 1 for target cells, 0 for empty
// This same mask is used for both rendering and scoring (single source of truth)
function buildScene7DragonMask() {
  const mask = Array.from({ length: GRID_ROWS }, () => Array(GRID_COLS).fill(0));
...
const SCENE7_THICKNESS_GAIN_MULTIPLIER = 2.28;
...
const SCENE7_FLOW_ENABLED = true;
const SCENE7_FLOW_THRESHOLD = 0.62;      // Medium-dark cells flow (0.62 = medium-dark brown)
const SCENE7_FLOW_HOLD_TIME_MS = 300;    // Must hold for 300ms before flow starts
const SCENE7_FLOW_RATE = 0.06;           // Faster relaxation for medium-dark sugar
const SCENE7_FLOW_MAX_PER_TICK = 0.018;  // Slightly more flow per tick

// Dark sugar flow constants (soften dark spots immediately)
const SCENE7_DARK_FLOW_ENABLED = true;
const SCENE7_DARK_FLOW_THRESHOLD = 0.42;   // Dark cells flow (0.42 = dark golden)
const SCENE7_DARK_TARGET_LEVEL = 0.34;     // Target level after flow
const SCENE7_DARK_FLOW_RATE = 0.14;        // Faster relaxation for dark sugar
const SCENE7_DARK_FLOW_MAX_PER_TICK = 0.035; // More flow per tick for dark sugar

// Guide shadow colors (faint, so player sugar appears on top)
const SCENE7_GUIDE_FILL = "rgba(185, 145, 55, 0.20)";
const SCENE7_GUIDE_EDGE = "rgba(145, 105, 35, 0.28)";
...
export class Scene7 {
  constructor(containerEl, onComplete) {
    ...
    this._lastDiagLogTime = 0; // For throttling diagnostic logs
    this.scene7ShowDrawDebugDot = false; // Debug dot disabled
    ...
    // - scene7StickTargetOverride: temporary T/red-dot debug pointer only.
    // - scene7CandyStickSharedAnchor: saved anchor used by BOTH stick flight and final candy placement.
    ...
    this.SCENE7_FINAL_CANDY_ANCHOR_X = 0.5;
    this.SCENE7_FINAL_CANDY_ANCHOR_Y = 0.46;

    // Task A: SHARED anchor for both stick flight and final candy placement
    // This is the single source of truth for both
    this.scene7CandyStickSharedAnchor = null;
    ...
    // Speed-based brush thickness
    this._speedHistory = [];
    this._maxSpeedHistory = 10;
    this._averageSpeed = 0;
    this._brushSpeedMultiplier = 1.0; // Default: no speed modification
    ...
    this._syrupPourAudio.volume = SCENE7_SYRUP_VOLUME;
    ...
    this._syrupPourAudio.addEventListener('loadeddata', () => {
      console.log('Scene7 syrup audio loaded:', this._syrupPourAudio.src);
    });
    this._syrupPourAudio.addEventListener('canplaythrough', () => {
      console.log('Scene7 syrup audio ready:', this._syrupPourAudio.src);
    });
    this._syrupPourAudio.addEventListener('error', (e) => {
      console.error('Scene7 syrup audio failed to load:', e, this._syrupPourAudio.src);
    });

    // Preload the audio
    this._syrupPourAudio.load();
    console.log('Scene7: Loading syrup pour audio from', this._syrupPourAudio.src);
    ...
    this._bgmAudio.volume = SCENE7_BGM_VOLUME;
    this._bgmAudio.preload = "auto";

    // Debugging event listeners for BGM
    this._bgmAudio.addEventListener('loadeddata', () => {
      console.log('Scene7 BGM loaded:', this._bgmAudio.src);
    });
    this._bgmAudio.addEventListener('canplaythrough', () => {
      console.log('Scene7 BGM ready:', this._bgmAudio.src);
    });
    this._bgmAudio.addEventListener('error', (e) => {
      console.error('Scene7 BGM failed to load:', e, this._bgmAudio.src);
    });

    // Preload BGM
    this._bgmAudio.load();
    console.log('Scene7: Loading BGM from', this._bgmAudio.src);

    // Task B & C: Production panel state
    this._showProductionPanel = false;
    this._productionQuantity = SCENE7_MIN_PRODUCTION_QUANTITY;
    ...
    this.scene7ShowProductionHitboxDebug = false;
    ...
    this._guideMask = buildScene7DragonMask();
  ...
  _getScene7CandyStickSharedAnchor() {
    // Priority 1: Use shared anchor from memory
    if (this._isValidTarget(this.scene7CandyStickSharedAnchor)) {
      return {
        x: this.scene7CandyStickSharedAnchor.x,
        y: this.scene7CandyStickSharedAnchor.y + 100  // Shift down by 100px in design coordinates
      };
    }

    // Priority 2: Load from localStorage
    try {
      const stored = localStorage.getItem("scene7CandyStickSharedAnchor");
      if (stored) {
        const parsed = JSON.parse(stored);
        if (this._isValidTarget(parsed)) {
          this.scene7CandyStickSharedAnchor = parsed; // Cache in memory
          return {
            x: parsed.x,
            y: parsed.y + 100  // Shift down by 100px in design coordinates
          };
        }
      }
    } catch (e) {
      console.warn('Scene7: Failed to load shared anchor from localStorage', e);
    }

    // Priority 3: Emergency fallback - use candy center (shifted down by 100px)
    console.warn("Scene7: No shared anchor found, using candy center fallback");
    return {
      x: this._candyCenterX || CANVAS_W / 2,
      y: (this._candyCenterY || CANVAS_H / 2) + 100  // Shift down by 100px in design coordinates
    };
  }
  ...
  _loadSharedAnchorFromStorage() {
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored);
        if (this._isValidTarget(parsed)) {
          this.scene7CandyStickSharedAnchor = parsed;
          console.log('Scene7: Loaded shared candy/stick anchor from localStorage', this.scene7CandyStickSharedAnchor);
          return;
        } else {
          localStorage.removeItem("scene7CandyStickSharedAnchor");
        }
      }
    } catch (e) {
      console.warn('Scene7: Failed to load shared candy/stick anchor from localStorage', e);
      localStorage.removeItem("scene7CandyStickSharedAnchor");
    }
  }
  ...
  _saveSharedAnchorToStorage() {
    try {
      localStorage.setItem("scene7CandyStickSharedAnchor", JSON.stringify(this.scene7CandyStickSharedAnchor));
      console.log('Scene7: Saved shared candy/stick anchor to localStorage', this.scene7CandyStickSharedAnchor);
    } catch (e) {
      console.warn('Scene7: Failed to save shared candy/stick anchor to localStorage', e);
    }
  }
  ...
  _startBGM() {
    ...
    if (playPromise !== undefined) {
      playPromise.then(() => {
        ...
        console.log("Scene7 BGM started");
      }).catch((e) => {
        console.warn('Scene7: BGM autoplay blocked, will retry on next user gesture', e);
  ...
  _stopBGM() {
    if (this._bgmAudio && this._bgmStarted) {
      ...
      this._bgmStarted = false;
      console.log("Scene7 BGM stopped");
    }
  }
  ...
  _loadStickTargetFromStorage() {
    ...
    try {
      const stored = localStorage.getItem("scene7StickTargetOverride");
      if (stored) {
        const parsed = JSON.parse(stored);
        // Only accept if both x and y are finite numbers
        if (this._isValidTarget(parsed)) {
          this.scene7StickTargetOverride = parsed;
          console.log('Scene7 loaded saved stick target', {
            x: this.scene7StickTargetOverride.x,
            y: this.scene7StickTargetOverride.y,
            source: "localStorage"
          });
        } else {
          // Clear invalid data
          localStorage.removeItem("scene7StickTargetOverride");
          console.warn('Scene7: Cleared invalid stick target from localStorage', parsed);
        }
      }
    } catch (e) {
      console.warn('Scene7: Failed to load stick target from localStorage', e);
      localStorage.removeItem("scene7StickTargetOverride");
    }
  }
  ...
  _saveStickTargetToStorage() {
    try {
      localStorage.setItem("scene7StickTargetOverride", JSON.stringify(this.scene7StickTargetOverride));
      console.log('Scene7: Saved stick target to localStorage');
    } catch (e) {
      console.warn('Scene7: Failed to save stick target to localStorage', e);
    }
  }
  ...
  _loadImages() {
    ...
    bg.onerror = () => { this._imagesLoaded++; console.warn("Scene7 image failed to load", bg.src); };
    ...
    bgNoSpoon.onerror = () => { this._imagesLoaded++; console.warn("Scene7 image failed to load", bgNoSpoon.src); };
    ...
    spoonBefore.onerror = () => { this._imagesLoaded++; console.warn("Scene7 image failed to load", spoonBefore.src); };
    ...
    spoonPouring.onerror = () => { this._imagesLoaded++; console.warn("Scene7 image failed to load", spoonPouring.src); };
    ...
    successImg.onload = () => { this._successScreenImg = successImg; console.log('Scene7: Success screen image loaded'); };
    successImg.onerror = () => { console.warn("Scene7 image failed to load", successImg.src); };
    successImg.src = 'assets/button7/successful-drawing.png';
  }
  ...
  _playPopSound() {
    try {
      ...
      console.log('Scene7: Pop sound failed (non-critical)', e);
    }
  }
  ...
  _initEvents() {
    ...
    this._onMouseDown = (e) => {
      ...
      const point = this._getScene7DesignPointFromClient(e.clientX, e.clientY);
      ...
      if (this._showSuccessScreen) {
        console.log("Scene7 success screen clicked, transitioning to next scene");
      ...
      if (this._showProductionPanel && !this._productionConfirmed && !this._productionAnimating) {
        ...
        if (this._isPointInScene7Box(this._minusBtnRect, canvasX, canvasY, designX, designY)) {
          this._productionQuantity = Math.max(SCENE7_MIN_PRODUCTION_QUANTITY, this._productionQuantity - 1);
          return;
        }

        // Check if clicked on plus button
        if (this._isPointInScene7Box(this._plusBtnRect, canvasX, canvasY, designX, designY)) {
          if (SCENE7_MAX_PRODUCTION_QUANTITY !== null && this._productionQuantity >= SCENE7_MAX_PRODUCTION_QUANTITY) {
            this._quantityFeedback = '客人只要一个龙形糖画，不能再多做了。';
        ...
        if (this._isPointInScene7Box(this._makeBtnRect, canvasX, canvasY, designX, designY)) {
          ...
          console.log("Scene7 starting production animation", {
      ...
      // Debug: Log mouse down diagnostic for drawing
      if (this.hasPickedUpSpoon && !this.completed) {
        const designPoint = this._getScene7DesignPointFromClient(e.clientX, e.clientY);
        console.log("Scene7 draw mousedown diagnostic", {
          ...
          inSheet: this._isInSheet(designPoint.x, designPoint.y),
          ladleX: this.ladleX,
          ladleY: this.ladleY
        });
      }
      ...
      if (this._phase === "stickPlacement" && this._stickState === "idle") {
        if (this._isStickClicked(designX, designY)) {
          ...
          const anchor = this._getScene7CandyStickSharedAnchor();
          this._stickTargetX = anchor.x;
          this._stickTargetY = anchor.y;

          // Task D: Invariant debug log - stick uses shared anchor
          console.log("Scene7 shared anchor invariant - stick", {
            sharedAnchor: this._getScene7CandyStickSharedAnchor(),
      ...
      if (this._syrupPourAudio && this._syrupPourAudio.paused) {
        this._syrupPourAudio.play().then(() => {
          ...
          console.log('Scene7: Audio unlocked successfully');
        }).catch((err) => {
          console.log('Scene7: Audio unlock attempt (normal):', err.name);
        });
      }
      ...
      if (this._productionConfirmed && this._showContinueHint) {
        console.log("Scene7 continuing to next cutscene", {
          quantity: this._productionQuantity
        });
    ...
    this._onMouseMove = (e) => {
      ...
      const designPos = this._getScene7DesignPointFromClient(e.clientX, e.clientY);
      this.scene7MouseX = designPos.x;
      this.scene7MouseY = designPos.y;
    ...
    this._onKeyDown = (e) => {
      ...
      if (e.key === 'T' || e.key === 't') {
        ...
        if (e.ctrlKey && e.shiftKey) {
          console.log("Scene7 Ctrl+Shift+T is deprecated. Use T then L to lock shared candy/stick anchor.");
          return;
        }
        // Regular T = Set temporary T target (debug/visual only, does NOT affect stick flight)
          ...
          if (this._phase === "drawing" || this._phase === "stickPlacement" || this._phase === "result") {
            ...
            this.scene7StickTargetOverride = {
              x: this.scene7MouseX,
              y: this.scene7MouseY
            };
      ...
      if (e.key === 'L' || e.key === 'l') {
        if (this._phase === "drawing" || this._phase === "stickPlacement" || this._phase === "result") {
          ...
          let tTarget = null;
          if (this._isValidTarget(this.scene7StickTargetOverride)) {
            tTarget = this.scene7StickTargetOverride;
          } else {
          ...
          if (this._isValidTarget(tTarget)) {
            ...
            // Read back and log
            const readBack = JSON.parse(localStorage.getItem("scene7CandyStickSharedAnchor"));
            console.log("Scene7 locked SHARED candy/stick anchor from T target", {
              tTarget,
              sharedAnchor: this.scene7CandyStickSharedAnchor,
              readBack
            });

            console.log("Scene7 L locked shared anchor", {
              sharedAnchor: this.scene7CandyStickSharedAnchor
            });

            this._showNotification('共享目标已锁定！棒子和糖果都会用这个位置！', '#00ff00', 120);
          } else {
            console.warn("Scene7: Cannot lock shared anchor - no valid T target. Press T first.");
    ...
    this._onTouchStart = (e) => {
      ...
      const point = this._getScene7DesignPointFromClient(t.clientX, t.clientY);
      const designX = point.x;
      const designY = point.y;
      ...
      // Task H: Handle success screen click - wait for user click then transition
      if (this._showSuccessScreen) {
        console.log("Scene7 success screen touched, transitioning to next scene");
        // Call onComplete to transition to next scene (Scene 6 or placeholder)
        if (this.onComplete) {
          this.onComplete();
        }
      ...
      if (this._productionConfirmed && this._showContinueHint) {
        console.log("Scene7 continuing to next cutscene (touch)", {
          quantity: this._productionQuantity
        });
        // TODO: Call next cutscene/transition here if available
        // For now, just reset the state
      ...
      if (this.hasStartedDrawing && !this.playerSubmitted && !this.completed && this._finishButtonRect) {
        ...
        console.log("Scene7 finish touch check", {
          designX,
          designY,
      ...
      if (this._phase === "stickPlacement" && this._stickState === "idle") {
        if (this._isStickClicked(designX, designY)) {
          ...
          console.log("Scene7 stick flight using shared anchor (touch)", {
    ...
    this._onTouchMove = (e) => {
      ...
      const designPos = this._getScene7DesignPointFromClient(t.clientX, t.clientY);
      this.scene7MouseX = designPos.x;
      this.scene7MouseY = designPos.y;
    };
  ...
  _onStrokeStart() {
    ...
    if (!this.hasStartedDrawing) {
      // FIX: Use _getScene7DesignPointFromClient() to correctly convert client coords to design coords
      const pos = this._getScene7DesignPointFromClient(this.mouseX, this.mouseY);
      ...
      console.log("Scene 7 drawing start", {
  ...
  _getResultMessage(score) {
    ...
    return "这次龙形糖画还不太完美，不过下次一定会更有气势。";
  }
  ...
  _onPlayerSubmit() {
    ...
    console.log('Scene7 final scoring:', scoring);
    console.log('Scene7 result message:', this._resultMessage);
    ...
    this._stickPlacementTimeout = setTimeout(() => {
      ...
      // Target will be set when stick is clicked (using _getScene7CandyStickSharedAnchor())

      // Restore normal cursor for stick placement phase
      this.canvas.style.cursor = 'default';

      console.log('Scene7: Stick placement phase started');
      console.log('Scene7: Stick target will be determined when stick is clicked');
  ...
  _getScene7DesignPointFromClient(clientX, clientY) {
    const rect = this.canvas.getBoundingClientRect();
  ...
  _getScene7CanvasPointFromDesign(x, y) {
    return {
  ...
  _isPointInScene7Box(box, canvasX, canvasY, designX, designY) {
    if (!box) return false;
  ...
  _applyViscousFlow(dt) {
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        if (v < SCENE7_FLOW_THRESHOLD) continue;

        // Condition 2: Player held in this cell long enough
        if (this._cellHoldTime[idx] < SCENE7_FLOW_HOLD_TIME_MS / 1000) continue;

        // Condition 3: Cell received direct pouring (not from flow)
        // This prevents flowed cells from becoming new flow sources
        if (this._directPourCount[idx] === 0) continue;

        const excess = v - SCENE7_FLOW_THRESHOLD;
        const flowAmount = Math.min(
          SCENE7_FLOW_MAX_PER_TICK,
          excess * SCENE7_FLOW_RATE * dtFrames
        );
  ...
  _applyDarkFlow(dt) {
    ...
    if (!SCENE7_DARK_FLOW_ENABLED) return;
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        if (v < SCENE7_DARK_FLOW_THRESHOLD) continue;

        // Calculate how much to flow (spread dark to light)
        const excess = v - SCENE7_DARK_TARGET_LEVEL;
        const flowAmount = Math.min(
          SCENE7_DARK_FLOW_MAX_PER_TICK,
          excess * SCENE7_DARK_FLOW_RATE * dtFrames
        );
        ...
        for (const ni of neighbors) {
          ...
          if (this.grid[ni] < SCENE7_DARK_FLOW_THRESHOLD) {
            deltas[ni] += each;
          }
        }
      }
  ...
  _shouldPlaySyrupAudio() {
    // FIX: Use _getScene7DesignPointFromClient() for correct coordinate conversion
    const pos = this._getScene7DesignPointFromClient(this.mouseX, this.mouseY);
  ...
  _updatePourAudio() {
    if (!this._syrupPourAudio) {
      console.warn('Scene7: No syrup pour audio object');
      return;
    }
    ...
    if (shouldPlay) {
      if (this._syrupPourAudio.paused) {
        console.log('Scene7 syrup audio START', {
          mouseDown: this.mouseDown,
          hasPickedUpSpoon: this.hasPickedUpSpoon,
          hasStartedDrawing: this.hasStartedDrawing,
          completed: this.completed
        ...
        if (playPromise !== undefined) {
          playPromise.then(() => {
            ...
            console.log('Scene7: Syrup pour audio started successfully');
          }).catch((err) => {
            console.error('Scene7: Audio play failed:', err.name, err.message);
          });
        } else {
          this._isPouringSoundPlaying = true;
        }
      ...
      if (!this._syrupPourAudio.paused) {
        console.log('Scene7: Pausing syrup pour audio');
        this._syrupPourAudio.pause();
        this._isPouringSoundPlaying = false;
      }
    }
  ...
  _updateCandidates(prevX, prevY, curX, curY, dtFrames) {
    // Diagnostic: log when updateCandidates is called (throttled)
    const now = performance.now();
    if (!this._lastCandidatesLogTime) this._lastCandidatesLogTime = 0;
    if (now - this._lastCandidatesLogTime > 500) {
      this._lastCandidatesLogTime = now;
      console.log("Scene7 updateCandidates called", {
        prevX,
        prevY,
        curX,
        curY,
    ...
    for (let i = 0; i < GRID_COLS * GRID_ROWS; i++) {
      ...
      // Spread amounts with precision tuning (SCENE7_SPREAD_STRENGTH = 0.55)
      // coreAmt uses SCENE7_THICKNESS_GAIN_MULTIPLIER for faster darkening
      // Task: Apply speed-based brush thickness multiplier
      let coreAmt, ring1Amt, ring2Amt;
      if (dwell >= SPREAD_BLOB_DWELL) {
        coreAmt = 0.10 * SCENE7_SPREAD_STRENGTH * SCENE7_THICKNESS_GAIN_MULTIPLIER;
        ring1Amt = 0.06 * SCENE7_SPREAD_STRENGTH;
        ring2Amt = 0;  // Disabled for smaller initial stamp (plus shape)
      } else if (dwell >= SPREAD_RING2_DWELL) {
        coreAmt = 0.13 * SCENE7_SPREAD_STRENGTH * SCENE7_THICKNESS_GAIN_MULTIPLIER;
        ring1Amt = 0.08 * SCENE7_SPREAD_STRENGTH;
        ring2Amt = 0;  // Disabled for smaller initial stamp (plus shape)
      } else {
        coreAmt = 0.16 * SCENE7_SPREAD_STRENGTH * SCENE7_THICKNESS_GAIN_MULTIPLIER;
        ring1Amt = 0.06 * SCENE7_SPREAD_STRENGTH;
        ring2Amt = 0;  // Disabled for smaller initial stamp (plus shape)
      }
  ...
  _update(dt) {
    ...
    if (this._phase === "drawing") {
      ...
      // Ladle = mouse position (FIX: use _getScene7DesignPointFromClient for correct coordinate conversion)
      const target = this._getScene7DesignPointFromClient(this.mouseX, this.mouseY);
      ...
      // Fast speed → thinner line (multiplier < 1)
      // Slow speed → thicker line (multiplier > 1)
      const SCENE7_BRUSH_SPEED_MIN = 2;  // Slow movement threshold
      const SCENE7_BRUSH_SPEED_MAX = 25; // Fast movement threshold
      const SCENE7_BRUSH_MULT_MIN = 0.4; // Minimum multiplier (fast = thinner)
      const SCENE7_BRUSH_MULT_MAX = 1.6; // Maximum multiplier (slow = thicker)
      
      let speedNorm = Math.max(0, Math.min(1, (this._averageSpeed - SCENE7_BRUSH_SPEED_MIN) / (SCENE7_BRUSH_SPEED_MAX - SCENE7_BRUSH_SPEED_MIN)));
      this._brushSpeedMultiplier = SCENE7_BRUSH_MULT_MAX - speedNorm * (SCENE7_BRUSH_MULT_MAX - SCENE7_BRUSH_MULT_MIN);
      ...
      // Diagnostic: throttle log every 500ms while mouse is held
      if (this.mouseDown && this.hasPickedUpSpoon && !this.completed) {
        const now = performance.now();
        if (now - this._lastDiagLogTime > 500) {
          this._lastDiagLogTime = now;
          console.log("Scene7 draw gate", {
            mouseDown: this.mouseDown,
            hasStartedDrawing: this.hasStartedDrawing,
            hasPickedUpSpoon: this.hasPickedUpSpoon,
            inSheet: inSheet,
            ladleX: this.ladleX,
            ladleY: this.ladleY,
            ladlePrevX: this.ladlePrevX,
            ladlePrevY: this.ladlePrevY
          });
        }
      }

      if (this.mouseDown && inSheet && this.hasStartedDrawing) {
        // Diagnostic: count candy cells before
        let _beforeCount = 0;
        for (let i = 0; i < GRID_COLS * GRID_ROWS; i++) { if (this.grid[i] > 0) _beforeCount++; }
        
        this._updateCandidates(this.ladlePrevX, this.ladlePrevY, this.ladleX, this.ladleY, dtFrames);
        
        // Diagnostic: count candy cells after
        let _afterCount = 0;
        for (let i = 0; i < GRID_COLS * GRID_ROWS; i++) { if (this.grid[i] > 0) _afterCount++; }
        
        if (_afterCount !== _beforeCount) {
          console.log("Scene7 candy write check", { before: _beforeCount, after: _afterCount, delta: _afterCount - _beforeCount });
        }
      ...
      // Viscous candy flow (slow diffusion for very thick candy)
      if (SCENE7_FLOW_ENABLED && !this.completed) {
        this._applyViscousFlow(dt);
      }

      // Dark sugar flow (soften dark spots immediately)
      if (SCENE7_DARK_FLOW_ENABLED && !this.completed) {
  ...
  _render() {
    ...
    if (!this._showFinalCandy) {
      for (let row = 0; row < GRID_ROWS; row++) {
        for (let col = 0; col < GRID_COLS; col++) {
          ...
          const visualV = Math.min(v, SCENE7_MAX_VISUAL_THICKNESS);
    ...
    // Temporary debug dot: prove render layer can show candy
    if (this.scene7ShowDrawDebugDot && this.mouseDown && this.hasPickedUpSpoon && !this.completed) {
      const inSheet = this._isInSheet(this.ladleX, this.ladleY);
      if (inSheet) {
        // Draw bright cyan dot at ladle position
        const dotX = this.ladleX;
        const dotY = this.ladleY;
    ...
    if (this.scene7ShowTargetDot && 
        this.scene7StickTargetOverride && 
        Number.isFinite(this.scene7StickTargetOverride.x) && 
        Number.isFinite(this.scene7StickTargetOverride.y)) {
      const dotX = this.scene7StickTargetOverride.x;  // Design coordinate — ctx already scaled
      const dotY = this.scene7StickTargetOverride.y;
      ctx.save();
    ...
    if (!this._showFinalCandy && (this._phase === "stickPlacement" || this._phase === "completed")) {
      if (this._stickLoaded && this._stickImg) {
        ...
        const SCENE7_STICK_RENDER_LENGTH = 330;  // px, scaled up by 50% from 220
        const SCENE7_STICK_ROTATION_RADIANS = 0;  // No rotation (image is already vertical)
        ...
        if (imgW > imgH) {
          ...
          renderedWidth = SCENE7_STICK_RENDER_LENGTH;
          renderedHeight = SCENE7_STICK_RENDER_LENGTH * imgH / imgW;
        } else {
          // Image is vertical (portrait), length = height
          renderedHeight = SCENE7_STICK_RENDER_LENGTH;
          renderedWidth = SCENE7_STICK_RENDER_LENGTH * imgW / imgH;
        }
        ...
        if (this.scene7StickTargetOverride && 
            Number.isFinite(this.scene7StickTargetOverride.x) && 
            Number.isFinite(this.scene7StickTargetOverride.y)) {
          console.log("Scene7 stick render/fly debug", {
            targetX: this._stickTargetX,
            targetY: this._stickTargetY,
            redDotX: this.scene7StickTargetOverride.x,
            redDotY: this.scene7StickTargetOverride.y,
            renderedWidth,
            renderedHeight,
            rotationRadians: SCENE7_STICK_ROTATION_RADIANS,
            renderMode: "canvas"
          });
        }
        ...
        ctx.rotate(SCENE7_STICK_ROTATION_RADIANS); // Rotate 90 degrees
    ...
    if (this._showFinalCandy && !this._showSuccessScreen && this._finalCandyLoaded && this._finalCandyImg) {
      ...
      // Task: Scale up by 50% for better visibility (160 * 1.5 = 240)
      // Task: Scale down by 10% to prevent oversizing (240 * 0.9 = 216)
      // Task: Scale up to match large dragon blueprint (216 → 390)
      // Task: Further scale up to 460px for better visual impact
      const SCENE7_FINAL_CANDY_WIDTH = 460; // px, increased to 460 for better scale
      
      // Task: Move dragon artwork slightly upward for better visual centering
      const SCENE7_FINAL_DRAGON_VISUAL_OFFSET_Y = -40; // px, move dragon up by 40px
      ...
      const baseScale = SCENE7_FINAL_CANDY_WIDTH / imgW; // Single scale factor
      const renderedWidth = imgW * baseScale; // = SCENE7_FINAL_CANDY_WIDTH
      ...
      const canvasDrawX = canvasAnchorX - finalWidth * this.SCENE7_FINAL_CANDY_ANCHOR_X;
      ...
      console.log('[CandyComplete] final candy image render position', {
        ...
        finalWidth,
        finalHeight,
        anchorX: this.SCENE7_FINAL_CANDY_ANCHOR_X,
        anchorY: this.SCENE7_FINAL_CANDY_ANCHOR_Y,
        popScale
      });
  ...
  _renderProductionPanel(ctx) {
    ...
    if (this.scene7ShowProductionHitboxDebug) {
      ctx.save();
  ...
  _updateStickAnimation(dt) {
    if (this._stickState === "flying") {
      ...
      if (this._stickAnimProgress >= 1) {
        ...
        this._finalCandyTimeout = setTimeout(() => {
          ...
          const sharedAnchor = this._getScene7CandyStickSharedAnchor();
          console.log("Scene7 shared anchor invariant - final candy", {
            sharedAnchor,
    ...
    if (this._showFinalCandy) {
      ...
      if (this._finalCandyAnimProgress >= 1 && !this._productionConfirmed && !this._showProductionPanel) {
        ...
        console.log("Scene7: Showing production panel");
      }
    }
  }
  ...
  _updateProductionAnimation() {
    ...
    if (this._productionIndex < this._productionQuantity &&
      ...
      console.log("Scene7 produced candy", {
    ...
    if (allDone && !this._productionComplete) {
      ...
      this._producedCandies = [];
      console.log("Scene7 production animation complete, showing success screen");
    }
  }
  ...
  _isStickClicked(designX, designY) {
    ...
    // Same calculation as rendering (SCENE7_STICK_RENDER_LENGTH = 140)
    const SCENE7_STICK_RENDER_LENGTH = 140;
    let renderedWidth, renderedHeight;
    if (imgW > imgH) {
      renderedWidth = SCENE7_STICK_RENDER_LENGTH;
      renderedHeight = SCENE7_STICK_RENDER_LENGTH * imgH / imgW;
